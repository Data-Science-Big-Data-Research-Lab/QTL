...
Examples:
  ...
  python runner.py --config config.yaml --study ablation --parallel 4 --machine-id gpu_node_1
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Filter by number of qubits for ablation study",
    )
    parser.add_argument(
        "--depth",
        type=int,
        default=None,
        help="Filter by circuit depth for ablation study",
    )
    parser.add_argument(
        "--noise-channels",
        type=str,
        default=None,
        help="Filter by noise channels (comma-separated) for noise decomposition",
    )
...
def prompt_overwrite(run_id: str, bulk_decision: list) -> bool:
    ...
    if bulk_decision[0] == "overwrite_all":
        ...
        return True
    # In non-interactive environments (e.g. SLURM) skip safely instead of crashing with EOFError
    import sys
    if not sys.stdin.isatty():
        logger.warning(f"No interactive terminal detected. Auto-skipping existing run: {run_id}")
        return False
    while True:
    ...
    if args.study:
        if args.study == "ablation":
            ...
            if n_qubits is not None:
                parts.append(f"q{n_qubits}")
            if depth is not None:
                parts.append(f"d{depth}")
        elif args.study == "noise_decomposition":
            parts.append("noise_decomp")
            noise_channels = getattr(args, "noise_channels", None)
            if noise_channels:
                parts.append(noise_channels.replace(",", "-"))
        elif args.study == "sim_as_hardware":
            parts.append("sim_as_hw")
        else:
            parts.append(args.study)

    if args.seed is not None:
        parts.append(f"seed{str(args.seed).replace(',', '-')}")
...
def apply_filters(runs: List[RunConfig], args: argparse.Namespace) -> List[RunConfig]:
    ...
    if args.qubits is not None:
        runs = [r for r in runs if r.overrides and r.overrides.get("n_qubits") == args.qubits]

    if args.depth is not None:
        runs = [r for r in runs if r.overrides and r.overrides.get("depth") == args.depth]

    if args.noise_channels is not None:
        allowed_channels = set(args.noise_channels.split(","))
        runs = [
            r for r in runs
            if r.overrides and "noise_channels" in r.overrides
            and set(r.overrides["noise_channels"]) == allowed_channels
        ]
...
def export_commands(
    ...
    for run in runs:
        ...
        if args.study:
            ...
            if run.overrides:
                if "n_qubits" in run.overrides and run.study == "ablation":
                    cmd.append(f"--qubits {run.overrides['n_qubits']}")
                if "depth" in run.overrides and run.study == "ablation":
                    cmd.append(f"--depth {run.overrides['depth']}")
                if "noise_channels" in run.overrides and run.study == "noise_decomposition":
                    channels_str = ",".join(run.overrides["noise_channels"])
                    cmd.append(f"--noise-channels {channels_str}")
...
def main() -> int:
    ...
    try:
        ...
        # Clean up empty/valueless run folder if no runs were completed and no errors occurred
        if completed == 0 and errors == 0 and os.path.exists(run_folder):
            try:
                has_valuable_data = False
                for root, dirs, files in os.walk(run_folder):
                    for f in files:
                        if f in ("runs.csv", "predictions.csv", "training_log.csv") or f.endswith(".pt") or f.endswith(".pth"):
                            has_valuable_data = True
                            break
                    if has_valuable_data:
                        break
                if not has_valuable_data:
                    import shutil
                    shutil.rmtree(run_folder)
                    logger.info(f"Cleaned up empty or duplicate run folder: {run_folder}")
            except Exception as e:
                logger.warning(f"Could not clean up run folder {run_folder}: {e}")
