...
COMMAND_FILES = {
    ...
    "5": ("cmds_qk_ideal.txt",    "Phase 5: Qiskit Ideal Only (Main & Ablation)"),
}
...
def _parse_run_id_from_cmd(line: str) -> Optional[str]:
    ...
    run_id format: {dataset}_{backbone}_{head}_{seed}  (main)
                   or {dataset}_{backbone}_{head}_q{n}_d{d}_{seed}  (ablation)
                   or {dataset}_{backbone}_{channel_name}_{seed}  (noise_decomposition)
                   or {dataset}_{backbone}_{head}_5ep_{seed}  (sim_as_hardware)
    """
    parts = line.split()
    d = b = h = s = None
    study = None
    ...
    for i, p in enumerate(parts):
        if p == "--dataset" and i + 1 < len(parts):
            ...
            study = parts[i + 1]
        elif p in ("--n-qubits", "--qubits") and i + 1 < len(parts):
            qubits = parts[i + 1]
        elif p == "--depth" and i + 1 < len(parts):
            depth = parts[i + 1]
        elif p == "--noise-channels" and i + 1 < len(parts):
            noise_channels = parts[i + 1]
            
    if d and b and h and s:
        if study == "ablation" and qubits and depth:
            return f"{d}_{b}_{h}_q{qubits}_d{depth}_{s}"
        elif study == "noise_decomposition" and noise_channels:
            # noise_channels is parsed as e.g. "amplitude_damping"
            return f"{d}_{b}_{noise_channels}_{s}"
        elif study == "sim_as_hardware":
            return f"{d}_{b}_{h}_5ep_{s}"
        elif not study or study == "main":
...
def refresh_commands():
    ...
    commands = [
        (f'"{sys.executable}" runner.py --config {CONFIG} --head linear,mlp_a,mlp_b --dry-run --export-commands > cmds_1_classical.txt', "Phase 1"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --head pl_ideal,qk_ideal --dry-run --export-commands > cmds_2_ideal.txt', "Phase 2"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --head pl_noisy,qk_noisy --dry-run --export-commands > cmds_3_noisy.txt', "Phase 3"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --study ablation --dry-run --export-commands > cmds_4_others.txt', "Phase 4 (Ablation)"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --study noise_decomposition --dry-run --export-commands >> cmds_4_others.txt', "Phase 4 (Noise)"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --study sim_as_hardware --dry-run --export-commands >> cmds_4_others.txt', "Phase 4 (Scalability)"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --head qk_ideal --dry-run --export-commands > cmds_qk_ideal.txt', "Phase 5 (Main QK-Ideal)"),
        (f'"{sys.executable}" runner.py --config {CONFIG} --study ablation --head qk_ideal --dry-run --export-commands >> cmds_qk_ideal.txt', "Phase 5 (Ablation QK-Ideal)"),
    ]
...
def show_monitoring():
    ...
    try:
        while True:
            ...
            completed_ids = set(df["run_id"].astype(str)) if df is not None and HAS_PANDAS and "run_id" in df.columns else _collect_completed_ids()
                        ...
                        try:
                            with open(file_path, encoding=enc) as f:
                                for line in f:
                                    ...
                                    n_total += 1
                                    run_id = _parse_run_id_from_cmd(line)
                                    if run_id and run_id in completed_ids:
                                        n_done += 1
                            break
                        except (UnicodeDecodeError, UnicodeError):
                            continue
...
def generate_tables_trigger():
    ...
    run_command(f'"{sys.executable}" generate_tables.py')
    input("\nPress Enter to return to menu...")
...
def main():
    ...
    while True:
        ...
        print(" [5] Launch Phase 5: Qiskit Ideal Only (Main & Ablation)")
